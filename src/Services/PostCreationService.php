...
class PostCreationService
{
    ...
    private function assertRequiredColumnsInMap(Collection $columnsToMap): void
    {
        foreach ($this->requiredRequestFields as $requiredRequestField) {
            $isMapped = $columnsToMap->contains(function ($mapping) use ($requiredRequestField) {
                return $this->resolveSourceRequestField($mapping) === $requiredRequestField;
            });
    ...
    private function assertRequiredColumnsInDatabase(Collection $columnsToMap, Collection $existingColumns): void
    {
        foreach ($this->requiredRequestFields as $requiredRequestField) {
            $targetColumns = $columnsToMap
                ->filter(function ($mapping) use ($requiredRequestField) {
                    return $this->resolveSourceRequestField($mapping) === $requiredRequestField;
                })
                ->keys();
     ...
     * @param  mixed  $mapping
     */
    private function resolveSourceRequestField($mapping): ?string
    {
        ...
        foreach ($columnsToMap as $columnToMapName => $columnToMap) {
            ...
            if (count($options)) {
                foreach ($options as $option) {
                    ...
                    if ($optionName === 'max_characters') {
                        $data[$columnToMapName] = Str::limit($data[$columnToMapName], max(0, (int) $optionValue - 3));
                    } elseif ($optionName === 'no-html') {
                        $data[$columnToMapName] = strip_tags($data[$columnToMapName]);
                    }
                }
            }
        }
